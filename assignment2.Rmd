...
We also apply $\chi^2$ test to both methods to compare their significance. 
...
From the result, we can find that the AUC scores for both models are quite high and backward stepwise selection performs slightly better than forward stepwise method, which is consistent with the conclusion from previous question. 
...
auc.B <- roc(train.y.mat ~ pred.B, plot = TRUE, 
             main = "ROC curves (on training set)", 
...
t(unique(rep.merged[abs(rep.merged$pos - rep.hit.SNP$pos[1]) <= 10^6 
                    & rep.merged$pos != rep.hit.SNP$pos[1], "gene"]))

# 1Mb window for 2nd SNP in hit.SNP
t(unique(rep.merged[abs(rep.merged$pos - rep.hit.SNP$pos[2]) <= 10^6 
...
From the scree plot, we observe that, from the PC4, the variance explained by each principle component keeps at a relatively low level. Thus, we consider using the first $3$ principle components in further steps.

We test the association between the outcome and each principle component by fitting a univariate logistic regression over the PC. According to the regression results, 
...
legend("bottomright", legend = c("cumulative proportion", "cut-off at 0.50"), 
