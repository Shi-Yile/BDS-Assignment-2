...
\begin{itemize}
  \item Lasso regression: $\lambda = 0.02982835$
  \item Ridge regression: $\lambda = 0.03677378$
\end{itemize}
...
\begin{enumerate}
  \item Low values of $\lambda$ except Ridge regression using \texttt{lambda\_1se} indicates the parameters do not get penalized hard.
  ...
  \item Using same kind of penalty parameters, Lasso regression has smaller values for both $\lambda$ and AUC than Ridge regression. In terms of AUC, we may consider that Ridge regression with \texttt{lambda\_min} preforms best on the training set ($0.983$), out of $4$ models.
  \item The model sizes of Lasso regression models, $8$ and $6$, are significantly smaller than the model sizes of Ridge regression models, which keep constant at $30$. This is because Ridge regression shrinks penalized coefficients near $0$ while Lasso regression shrinks them to $0$, based on the penalty parameter $\lambda$.
\end{enumerate}
...
We also apply $\chi^2$ test to both methods to compare their model significance to check our conclusions drawn from AIC values. As we expect, model.B has a lower $\chi^2$ statistics than model.S, indicating backward stepwise selection is more significant than forward stepwise selection, which is consistent with our previous conclusions.
...
```{r, warning = FALSE, error = FALSE, message = FALSE, fig.height = 3.5}
...
As required, we find the SNP with the smallest regression p-value, i.e. $rs12243326\_A$, which is considered to be mostly strongly associated to increased risk of gestational diabetes. Besides statistics in `rep`, we also report its 95% and 99% confidence intervals. Since the confidence intervals do not include $1$, we have sufficient evidence to conclude that $rs12243326\_A$ is strongly associated with the outcome of interest, consistent with our previous conclusion.

In addition, we report statistics including 95% and 99% confidence intervals for the SNP with most significant protective effect on gestational diabetes, i.e. $rs11575839\_C$, which has the most negative regression coefficient. Note that the confidence intervals include $1$, which suggests insignificance of $rs11575839\_C$, consistent with its large p-value of $0.1090394$.
...
\begin{itemize}
  \item \texttt{rs12243326}: $TCF7L2$
  \item \texttt{rs2237897}: $TH$, $KCNQ1$, $ACNA2D4$, $SMG6$
\end{itemize}
...
log.lld.test.hit <- sum(dbinom(gdm.test$pheno, size = 1, 
                               prob = predict.test.hit.SNP, log = TRUE))
log.lld.test.p3 <- sum(dbinom(gdm.test$pheno, size = 1, 
                               prob = predict.test.p3.SNP, log = TRUE))
log.lld.test.FTO <- sum(dbinom(gdm.test$pheno, size = 1, 
                               prob = predict.test.FTO.SNP, log = TRUE))

# report the test log-likelihood
data.frame(score.model = c("score.hit", "score.p3", "score.FTO"),
           test.log.likelihood = c(log.lld.test.hit, log.lld.test.p3, 
                                   log.lld.test.FTO))
```
...
\begin{itemize}
  \item PC2 is insignificant for both unadjusted and adjusted models 
  \item The first principle component is significant in unadjusted model, while it becomes insignificant after adjusting on the aforementioned variables. 
  \item Principle component $3$ keeps its significance in both models.
\end{itemize}
...
legend("bottomright", 
       legend = c("cumulative proportion", "cut-off at 0.5", "cut-off at 0.9"),        col = c("black", "blue", "red"), lty = c(1, 4, 4))
...
We start with Lasso regression and Ridge regression on two variables sets. Since there are factors in the independent variable set, we define and apply a helper function `prepare.gllmnet()` (from Lab 4) to covert the corresponding data table to a matrix. 

Next, as what we did in Problem 1, we use `createDataPartion()` function to split the training and testing sets with a ratio of $7:3$. After that, we run Lasso and Ridge regressions using cross-validation over all independent variables and gene expression variables only. To compare the model performances, we get the maximised AUC of each model with the optimal $\lambda$. 
...
In conclusion, we pick up \textbf{Ridge regression over all variables} as the most accurate model with respect to AUC score.
...
data.frame(modelsize = c(fit.cv.Lasso$nzero[fit.cv.Lasso$index[1]],
                         fit.cv.Ridge$nzero[fit.cv.Ridge$index[1]], 
