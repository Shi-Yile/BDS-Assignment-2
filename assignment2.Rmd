...
train.x.mat <- as.matrix(train.wdbc[, -c(1)])
# convert strings in 'diagnosis' to numeric
...
rep.dt <- data.table(model = c("Lasso", "Ridge"),
                     lambda.min = c(round(fit.cv.lasso$lambda.min, 3), 
                                    round(fit.cv.ridge$lambda.min, 3)),
                     AUC.lambda.min = c(round(fit.cv.lasso$cvm[fit.cv.lasso$index[1]], 3),
                                        round(fit.cv.ridge$cvm[fit.cv.ridge$index[1]], 3)),
                    
                     lambda.1se = c(round(fit.cv.lasso$lambda.1se, 3),
                                    round(fit.cv.ridge$lambda.1se, 3)),
                     AUC.lambda.1se = c(round(fit.cv.lasso$cvm[fit.cv.lasso$index[2]], 3),
...
data.frame(abs.coef = sort(abs(scale(model.B$coefficients))[,1], 
                           decreasing = TRUE))
```
```{r}
# selected variables with standardized regression coefficients in model.S
cat("selected variables in model.S: ")
data.frame(abs.coef = sort(abs(scale(model.S$coefficients))[,1], 
                           decreasing = TRUE))
```
...
We use the deviance of the model to test the goodness-of-fit of the model by deriving a p-value.
...
cat("AUC of Model.B: ", auc.B, "\n")
cat("AUC of Model.S:, ", auc.S)
```
...
roc.Lasso <- roc(test.wbdc$diagnosis, pred.Lasso.test, 
                 plot = TRUE, col = "blue", xlim = c(0, 1), main = "ROC curves")
...
# create a data frame to show the AUC of each model
data.frame(AUC = c(roc.Lasso$auc, roc.Ridge$auc, roc.B$auc, roc.S$auc), 
